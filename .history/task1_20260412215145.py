...
def test_select_country(driver):
    ...
    # 1. Пытаемся закрыть возможный рекламный баннер (он часто мешает)
    try:
        close_button = driver.find_element(By.XPATH, "//button[contains(@class, 'close') or contains(@aria-label, 'Close')]")
        close_button.click()
        print("Рекламный баннер закрыт")
        time.sleep(1)
    except:
        print("Рекламного баннера не найдено")
