...
def test_main(driver):    
    ...
    #Развернуть список
    products = (By.XPATH , '//nav[not(contains(@class, "module"))]/ul/li[1]')
    element = driver.find_element(*products)
    element.click()
    #Тыкнуть доки
    docs_locator = ((By.XPATH, '//nav[not(contains(@class, "module"))]/ui/li[1]/ul/li[last()]'))
    wait = WebDriverWait(driver, timeout = 60, poll_frequency = 15)
    wait.until(EC.visibility_of_all_elements_located(docs_locator))
    element.click =()
